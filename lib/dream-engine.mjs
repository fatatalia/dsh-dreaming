...
export class DreamEngine {
  ...
  setConfig({ workspace, provider, model, windowStart, windowEnd } = {}) {
    ...
    this.log?.info?.(`dreaming: 配置更新 workspace=${this.workspace} provider=${this.provider || "(全局默认)"} model=${this.model || "(全局默认)"} 窗口=${this.windowStart}-${this.windowEnd}`);
    this.scheduleNext();
  }
